...
Implemented in `src/HAgent.Core/Models/AiPlanContracts.cs` with focused tests in `tests/HAgent.Tests/PlanContractsTests.cs` and a matching scenario in `src/HAgent.Example/MainForm.PlanContractsTests.cs`.
...
Example organization/registration still needs to expose `PLAN CONTRACTS` under `Cognition → Goals & Plans` before this slice can enter user verification.

### Verification checkpoint

**Example to run:** `HAgent.Example → Cognition → Goals & Plans → PLAN CONTRACTS` on .NET Framework 4.8.1 and .NET 9 Windows, after registration is completed.
