...
namespace HAgent.Example
{
    internal sealed partial class MainForm
    {
        ...
        {
            if (Interlocked.Exchange(ref _executionTargetCatalogTabsAdded, 1) != 0)
                return;
        ...
        private async Task TestExecutionTargetCatalogAsync(string unused)
        {
            ...
            if (targets.Count != 2)
                throw new InvalidOperationException("Target catalog did not materialize the discovered models.");

            var supported = FindTarget(targets, provider.Id + "::catalog-model-supported");
            if (supported.Cost != AiCostStatus.Free ||
                supported.LogicalModelId != "logical-catalog-supported" ||
                supported.Capabilities.Get(AiCapability.Chat) != CapabilitySupport.Supported)
            ...
            if (unknown.Cost != AiCostStatus.Unknown ||
                unknown.Capabilities.Get(AiCapability.Chat) != CapabilitySupport.Unknown)
        ...
        private sealed class TargetCatalogTestAdapter : IAiProviderAdapter, IProviderDiscovery
        {
            ...
            public Task<ProviderDiscoveryResult> DiscoverAsync(AiProvider provider, string apiKey, CancellationToken cancellationToken)
            {
                ...
                var result = new ProviderDiscoveryResult
                {
                    Succeeded = true,
                    IsPartial = false,
                    Message = "Deterministic target catalog discovery."
                };
                result.Models.Add(new AiModelMetadata
                {
                    ProviderId = provider.Id,
                    ModelId = "catalog-model-supported",
                    LogicalModelId = "logical-catalog-supported",
                    DisplayName = "Supported Catalog Model",
                    Cost = AiCostStatus.Free,
                    Source = AiMetadataSource.ProviderMetadata,
                    Capabilities = supportedCapabilities
                });
                result.Models.Add(new AiModelMetadata
                {
                    ProviderId = provider.Id,
                    ModelId = "catalog-model-unknown",
                    LogicalModelId = "logical-catalog-unknown",
                    DisplayName = "Unknown Catalog Model",
