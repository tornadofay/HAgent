...
ALTER TABLE HAgentAgents ADD COLUMN IF NOT EXISTS CapabilityRequirementsJson longtext NULL;";
            ...
            using (var cmd = new MySqlCommand(sql, c))
            {
                ...
                using (var r = await cmd.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false))
                    while (await r.ReadAsync(cancellationToken).ConfigureAwait(false))
                        ...
                        {
                            Id = r.GetString(0), Name = r.GetString(1), Kind = r.GetString(2), BaseUrl = r.GetString(3),
                            DefaultModel = r.IsDBNull(4) ? string.Empty : r.GetString(4),
                            DefaultSystemPrompt = r.IsDBNull(5) ? string.Empty : r.GetString(5),
                            SecretId = r.IsDBNull(6) ? string.Empty : r.GetString(6), Enabled = r.GetBoolean(7)
                        });
            }
        ...
        public async Task<IReadOnlyList<AiAgent>> GetAgentsAsync(CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            const string sql = "SELECT Id,Name,SystemPrompt,UseProviderSystemPrompt,Temperature,MaxOutputTokens,Enabled,ToolIdsJson,ExecutionSelectionJson,CapabilityRequirementsJson FROM HAgentAgents ORDER BY Name";
            using (var c = new MySqlConnection(_connectionString))
            using (var cmd = new MySqlCommand(sql, c))
            {
                await c.OpenAsync(cancellationToken).ConfigureAwait(false);
                using (var r = await cmd.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false))
                {
                    while (await r.ReadAsync(cancellationToken).ConfigureAwait(false))
                    {
                        var agent = new AiAgent
                        {
                            ...
                            SystemPrompt = r.IsDBNull(2) ? string.Empty : r.GetString(2),
                            UseProviderSystemPrompt = r.GetBoolean(3),
                            Temperature = r.IsDBNull(4) ? (double?)null : r.GetDouble(4),
                            MaxOutputTokens = r.IsDBNull(5) ? (int?)null : r.GetInt32(5),
                            Enabled = r.GetBoolean(6)
                        };

                        DeserializeInto(r.IsDBNull(7) ? null : r.GetString(7), agent.ToolIds);
                        var selection = Deserialize<AiExecutionSelectionPolicy>(r.IsDBNull(8) ? null : r.GetString(8));
                        if (selection != null) agent.ExecutionSelection = selection;
                        var requirements = Deserialize<AiCapabilityRequirements>(r.IsDBNull(9) ? null : r.GetString(9));
        ...
        {
            using (var c = new MySqlConnection(_connectionString))
            using (var cmd = new MySqlCommand("SELECT ExecutionSelectionJson FROM HAgentAgents", c))
            {
                await c.OpenAsync(t).ConfigureAwait(false);
                using (var r = await cmd.ExecuteReaderAsync(t).ConfigureAwait(false))
                {
                    while (await r.ReadAsync(t).ConfigureAwait(false))
                    {
                        var selection = Deserialize<AiExecutionSelectionPolicy>(r.IsDBNull(0) ? null : r.GetString(0));
                        if (selection != null && string.Equals(selection.PreferredProviderId, id, StringComparison.OrdinalIgnoreCase))
        ...
        public async Task DeleteAgentAsync(string id, CancellationToken t = default(CancellationToken))
        {
            await ExecuteAsync("DELETE FROM HAgentAgents WHERE Id=@id", id, t).ConfigureAwait(false);
        }
        ...
        private static void BindProvider(MySqlCommand c, AiProvider p)
        {
            ...
            c.Parameters.AddWithValue("@BaseUrl", p.BaseUrl); c.Parameters.AddWithValue("@DefaultModel", p.DefaultModel);
            c.Parameters.AddWithValue("@DefaultSystemPrompt", p.DefaultSystemPrompt); c.Parameters.AddWithValue("@SecretId", p.SecretId); c.Parameters.AddWithValue("@Enabled", p.Enabled);
        }
        ...
        private static void BindAgent(MySqlCommand c, AiAgent a)
        {
            ...
            c.Parameters.AddWithValue("@SystemPrompt", a.SystemPrompt); c.Parameters.AddWithValue("@UseProviderSystemPrompt", a.UseProviderSystemPrompt);
