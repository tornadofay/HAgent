...
namespace HAgent.WinForms.Forms
{
    internal sealed class ToolEditorForm : HAgentForm
    {
        ...
        private void Build()
        {
            var layout = new TableLayoutPanel { Dock = DockStyle.Fill, ColumnCount = 2, RowCount = 5, BackColor = Color.FromArgb(248,250,252) };
            ...
            var footer = new FlowLayoutPanel { Dock = DockStyle.Bottom, Height = 46, FlowDirection = FlowDirection.RightToLeft, WrapContents = false, BackColor = Color.FromArgb(248,250,252) };
        ...
        private static void AddField(TableLayoutPanel layout, int row, string title, string description, Control control)
        {
            ...
            labels.Controls.Add(new Label { Text = description, Dock = DockStyle.Fill, Font = new Font("Segoe UI",8.1f), ForeColor = Color.FromArgb(100,116,139) });
            labels.Controls.Add(new Label { Text = title, Dock = DockStyle.Top, Height = 22, Font = new Font("Segoe UI",9.5f,FontStyle.Bold), ForeColor = Color.FromArgb(15,23,42) });
        ...
        private void Save()
        {
            if (string.IsNullOrWhiteSpace(_name.Text)) { MessageBox.Show(this,"Tool name is required.","HAgent",MessageBoxButtons.OK,MessageBoxIcon.Warning); return; }
            if (string.IsNullOrWhiteSpace(_description.Text)) { MessageBox.Show(this,"Tool description is required.","HAgent",MessageBoxButtons.OK,MessageBoxIcon.Warning); return; }
            if (string.IsNullOrWhiteSpace(_schema.Text)) { MessageBox.Show(this,"Input schema is required.","HAgent",MessageBoxButtons.OK,MessageBoxIcon.Warning); return; }
