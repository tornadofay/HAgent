...
namespace HAgent.Providers.OpenAICompatible
{
    public sealed partial class OpenAICompatibleProviderAdapter : IAiProviderAdapter, IProviderConnectionTester, IProviderModelCatalog, IProviderModelCapabilities, IProviderStreamingAdapter, IProviderToolCallingAdapter
    {
        ...
        public OpenAICompatibleProviderAdapter(HttpClient httpClient = null) { _httpClient = httpClient ?? new HttpClient(); }
        public string Kind => ProviderKind;
        public string DisplayName => "OpenAI-Compatible";

        public bool CanHandle(AiProvider provider)
        {
            return provider != null && string.Equals(provider.Kind, ProviderKind, StringComparison.OrdinalIgnoreCase);
        }

        public Task<AiModelCapabilities> GetCapabilitiesAsync(AiProvider provider, string model, string apiKey, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (provider == null) throw new ArgumentNullException(nameof(provider));
            var result = new AiModelCapabilities { Model = model ?? string.Empty };
            result.Set(AiCapability.Chat, CapabilitySupport.Supported, CapabilitySource.AdapterKnowledge, 0.95d, "The OpenAI-compatible adapter establishes support for the chat transport shape.");
            result.Set(AiCapability.Streaming, CapabilitySupport.Supported, CapabilitySource.AdapterKnowledge, 0.90d, "The adapter supports OpenAI-compatible Server-Sent Events streaming.");
        ...
        public async Task TestConnectionAsync(AiProvider provider, string apiKey, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (models == null) throw new InvalidOperationException("The provider returned no model catalog.");
        }
        ...
        public async Task<IReadOnlyList<string>> GetModelsAsync(AiProvider provider, string apiKey, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            using (var request = new HttpRequestMessage(HttpMethod.Get, url))
            {
                ...
                using (var response = await _httpClient.SendAsync(request, cancellationToken).ConfigureAwait(false))
                {
                    ...
                    if (!response.IsSuccessStatusCode) throw new HttpRequestException("AI provider model request returned " + (int)response.StatusCode + ": " + body);
                    ...
                    if (dto != null && dto.Data != null)
                    {
                        foreach (var item in dto.Data)
                        {
                            ...
                            if (result.FindIndex(x => string.Equals(x, item.Id, StringComparison.OrdinalIgnoreCase)) < 0) result.Add(item.Id);
                        }
                    }
            ...
            CancellationToken cancellationToken)
        {
            ...
            using (var httpRequest = new HttpRequestMessage(HttpMethod.Post, url))
            {
                ...
                if (!string.IsNullOrWhiteSpace(apiKey)) httpRequest.Headers.Authorization = new AuthenticationHeaderValue("Bearer", apiKey);
                using (var response = await _httpClient.SendAsync(httpRequest, cancellationToken).ConfigureAwait(false))
                {
                    ...
                    if (dto == null || dto.Choices == null || dto.Choices.Count == 0) throw new InvalidOperationException("The AI provider returned no choices.");
                    ...
                    if (dto.Usage != null)
                    {
                        ...
                        if (dto.Usage.PromptTokensDetails != null) usage["prompt_tokens_details"] = dto.Usage.PromptTokensDetails;
                        if (dto.Usage.CompletionTokensDetails != null) usage["completion_tokens_details"] = dto.Usage.CompletionTokensDetails;
                    }
                    ...
                    if (!string.IsNullOrWhiteSpace(content) && content.IndexOf("<think>", StringComparison.OrdinalIgnoreCase) >= 0) metadata["reasoning_markup_detected"] = true;
                ...
                using (var response = await _httpClient.SendAsync(httpRequest, HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false))
                {
                    ...
                    using (var reader = new StreamReader(stream))
                    {
                        ...
                        while (true)
                        {
                            ...
                            try { chunk = JsonConvert.DeserializeObject<StreamResponse>(payload); }
                            ...
                            foreach (var choice in chunk.Choices)
                            {
                                ...
                                if (choice.Delta.ToolCalls != null)
                                {
                                    foreach (var call in choice.Delta.ToolCalls)
                                    {
                                        ...
                                        if (!toolBuilders.TryGetValue(call.Index, out builder)) { builder = new StreamingToolBuilder(); toolBuilders.Add(call.Index, builder); }
                                ...
                                if (progress != null && (!string.IsNullOrEmpty(textDelta) || !string.IsNullOrEmpty(reasoningDelta)))
                                    progress.Report(new AIResponseDelta { Text = textDelta, Reasoning = reasoningDelta });
        ...
        private static AIUsage NormalizeUsage(UsageDto usage)
        {
            ...
            if (usage.PromptTokensDetails != null) result.CachedPromptTokens = usage.PromptTokensDetails.CachedTokens;
            if (usage.CompletionTokensDetails != null) result.ReasoningTokens = usage.CompletionTokensDetails.ReasoningTokens;
            ...
            if (usage.PromptTokensDetails != null) result.ProviderUsage["prompt_tokens_details"] = usage.PromptTokensDetails;
            if (usage.CompletionTokensDetails != null) result.ProviderUsage["completion_tokens_details"] = usage.CompletionTokensDetails;
            return result;
        }
        ...
        private static List<AIToolCall> NormalizeToolCalls(IReadOnlyList<ResponseToolCallDto> calls)
        {
            ...
            {
                if (call == null || call.Function == null || string.IsNullOrWhiteSpace(call.Function.Name)) continue;
                result.Add(new AIToolCall(call.Id ?? string.Empty, call.Function.Name, call.Function.Arguments ?? string.Empty));
            }
            return result;
        }
        ...
        private static bool IsJsonDocument(string text)
        {
            ...
            try { JsonConvert.DeserializeObject(value); return true; }
            catch (JsonException) { return false; }
        }

        private static string NormalizeEndpoint(string baseUrl)
        {
            if (string.IsNullOrWhiteSpace(baseUrl)) throw new InvalidOperationException("Provider Base URL is required.");
        ...
        private static List<RequestChatMessageDto> ToRequestDtos(IReadOnlyList<AIMessage> messages, string systemPrompt)
        {
            ...
            if (!string.IsNullOrWhiteSpace(systemPrompt)) result.Add(new RequestChatMessageDto { Role = "system", Content = systemPrompt });
            if (messages != null)
                foreach (var message in messages)
                    if (message != null) result.Add(new RequestChatMessageDto { Role = message.Role, Content = message.Content });
            return result;
        }
        ...
        private sealed class ChatCompletionRequest
        {
            ...
            [JsonProperty("messages")] public List<RequestChatMessageDto> Messages { get; set; }
        ...
        private sealed class ResponseChatMessageDto
        {
            ...
            [JsonProperty("tool_calls")] public List<ResponseToolCallDto> ToolCalls { get; set; }
        }
        private sealed class ResponseToolCallDto
        {
            [JsonProperty("id")] public string Id { get; set; }
            [JsonProperty("type")] public string Type { get; set; }
            [JsonProperty("function")] public ResponseToolFunctionDto Function { get; set; }
        }
        ...
        {
            [JsonProperty("id")] public string Id { get; set; }
            [JsonProperty("choices")] public List<ChoiceDto> Choices { get; set; }
            [JsonProperty("usage")] public UsageDto Usage { get; set; }
        }
        private sealed class ChoiceDto
        {
            [JsonProperty("message")] public ResponseChatMessageDto Message { get; set; }
        }
        private sealed class StreamResponse
        {
            [JsonProperty("id")] public string Id { get; set; }
            [JsonProperty("choices")] public List<StreamChoiceDto> Choices { get; set; }
            [JsonProperty("usage")] public UsageDto Usage { get; set; }
        }
        private sealed class StreamChoiceDto
        {
            [JsonProperty("index")] public int Index { get; set; }
            [JsonProperty("delta")] public StreamDeltaDto Delta { get; set; }
        }
        private sealed class StreamDeltaDto
        {
            ...
            [JsonProperty("content")] public string Content { get; set; }
            [JsonProperty("reasoning_content")] public string ReasoningContent { get; set; }
            [JsonProperty("tool_calls")] public List<StreamToolCallDto> ToolCalls { get; set; }
        }
        ...
        private sealed class StreamingToolBuilder
        {
            ...
            public StreamingToolBuilder() { Id = string.Empty; Name = string.Empty; Arguments = new StringBuilder(); }
        }
        private sealed class ModelListResponse
        {
            [JsonProperty("data")] public List<ModelDto> Data { get; set; }
        }
        ...
        {
            [JsonProperty("prompt_tokens")] public long? PromptTokens { get; set; }
            [JsonProperty("completion_tokens")] public long? CompletionTokens { get; set; }
            [JsonProperty("total_tokens")] public long? TotalTokens { get; set; }
            [JsonProperty("prompt_tokens_details")] public PromptTokensDetailsDto PromptTokensDetails { get; set; }
            [JsonProperty("completion_tokens_details")] public CompletionTokensDetailsDto CompletionTokensDetails { get; set; }
        }
        private sealed class PromptTokensDetailsDto
        {
            [JsonProperty("cached_tokens")] public long? CachedTokens { get; set; }
        }
        private sealed class CompletionTokensDetailsDto
        {
            [JsonProperty("reasoning_tokens")] public long? ReasoningTokens { get; set; }
        }
    }
}