...
{
    public sealed class MySqlExecutionAuditStore : IExecutionAuditStore
    {
        private readonly string _connectionString;

        public MySqlExecutionAuditStore(string connectionString)
        {
...
WHERE (@ExecutionId = '' OR ExecutionId = @ExecutionId)
  AND (@CorrelationId = '' OR CorrelationId = @CorrelationId)
...
LIMIT @MaxResults;";
        ...
        private static void Bind(MySqlCommand command, AgentExecutionAuditRecord record)
        {
            ...
            command.Parameters.AddWithValue("@StartedAt", record.StartedAt.HasValue ? (object)record.StartedAt.Value.UtcDateTime : DBNull.Value);
            command.Parameters.AddWithValue("@CompletedAt", record.CompletedAt.HasValue ? (object)record.CompletedAt.Value.UtcDateTime : DBNull.Value);
        ...
        private static AgentExecutionAuditRecord Read(MySqlDataReader reader)
        {
            ...
            return new AgentExecutionAuditRecord
            {
                ...
                FailureKind = failure,
                ProviderErrorKind = providerError,
                CreatedAt = reader.GetDateTime(10).ToUniversalTime(),
                StartedAt = reader.IsDBNull(11) ? (DateTimeOffset?)null : reader.GetDateTime(11).ToUniversalTime(),
                CompletedAt = reader.IsDBNull(12) ? (DateTimeOffset?)null : reader.GetDateTime(12).ToUniversalTime(),
