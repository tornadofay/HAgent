    ...
    internal sealed partial class MainForm
    {
        private void AddGoalIntentionContractsTab()
        {
            AddApiTab(
                ...
                "Goals and intentions remain distinct durable concepts; host-supplied state remains identifiable as host input, while agent-inferred state carries explicit provenance. Intention status changes preserve a reason and revision boundary.",
        ...
        private Task TestGoalIntentionContracts(string message)
        {
            ...
            hostGoal.Constraints.Add("Respect host constraints.");
            hostGoal.Validate();
            ...
            if (inferredGoal.Provenance.Authority != AiGoalAuthority.AgentInferred)
                throw new InvalidOperationException("Agent-inferred goal authority was not incorrectly treated as host input.");
