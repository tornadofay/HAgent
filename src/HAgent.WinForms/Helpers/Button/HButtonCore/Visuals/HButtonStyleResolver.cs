...
namespace HAgent.WinForms.Helpers.Button.HButtonCore.Visuals
{
    public static class HButtonStyleResolver
    {
        public static ButtonStyleSnapshot Resolve(in HButtonRenderContext ctx)
        {
            var state = ctx.State.VisualState;
            //   bool isCustom = ctx.Appearance.Style == ThemeStyle.Custom;
